...
%% GPS传感器2
sensor2.X_noise = 0.5;           %GPS的横向噪声方差
sensor2.Y_noise = 0.5;           %GPS的纵向噪声方差

R2 = diag([sensor2.X_noise, sensor2.Y_noise]); % 测量噪音的协方差矩阵
...
subplot(122);
plot(sensor1_X, sensor1_Y, '-');hold on
plot(sensor2_X, sensor2_Y, '-');
...
for k = 1:ms
    ...
    [KF_State(:,k+1), P2] = Track_KF(P2, A, C, Q, R2, KF_State(:,k), Sensor_Meas1); 
    %调用卡尔曼滤波函数更新估计状态和估计均方误差矩阵
