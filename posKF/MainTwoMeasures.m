...
car.x_noise = 0.05;                % car运动的横向坐标偏差的方差
car.y_noise = 0.05;                % car运动的纵向坐标偏差的方差
...
R2 = diag([sensor2.X_noise, sensor2.Y_noise]) * 0.5; % 测量噪音的协方差矩阵
...
P2 = 0.01 * eye(4); %eye(4)代表4阶单位矩阵，这里是对估计均方误差矩阵Pk赋初值
...
for k = 1:ms
    ...
    Sensor_Meas2 = [sensor2_X(1,k);sensor2_Y(1,k)]; % 测量向量Zk
...
figure;subplot(131);
plot(actual_state(1,:),actual_state(2,:), 'r','LineWidth',2); hold on
plot(sensor1_X, sensor1_Y, 'b-');hold on
plot(sensor2_X, sensor2_Y, 'g-');
legend('actual','sensor1','sensor2');
